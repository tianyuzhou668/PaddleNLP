        ...
        for f_id in range(math.ceil(len(files) / worker_num)):
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                if global_step % args.logging_steps == 0:
                    if worker_index == 0:
                        logger.info(
                            "global step %d, epoch: %d, batch: %d, loss: %f, speed: %.2f step/s, ips: %.2f tokens/s, learning rate: %.9f"
                            % (global_step, epoch, step, loss_return[0],
                               args.logging_steps / (time.time() - tic_train),
                               args.logging_steps * 1024 * args.batch_size *
                               worker_num / (time.time() - tic_train),
                               lr_return[0]))
