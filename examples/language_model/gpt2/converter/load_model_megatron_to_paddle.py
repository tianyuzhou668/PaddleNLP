...
def trans_megatron_to_paddle(path):
    paddle.set_device("cpu")
    paddle.disable_static()
    state = load_megatron_data(path)
    new_state_dict = {}
    all_num = 0
    for key in sorted(list(state.keys())):
        ...
        if "query_key_value" in key:
            ...
            if "weight" in key:
                ...
                print("BIAS SHAPE", state[key].shape,
                      state[key].transpose().shape)
                #state[key] = state[key].transpose().reshape(heads, shape[0]//heads)
                q, k, v = np.split(state[key], 3, axis=-1)
                q = q.reshape((-1))
                k = k.reshape((-1))
...
if __name__ == "__main__":
    new_state_dict = trans_megatron_to_paddle(sys.argv[1])
    for key, value in new_state_dict.items():
        print("key:{}, shape:{}, dtype:{}".format(key, value.shape,
                                                  value.dtype))
